...
const uploadPhoto = async (file) => {
  ...
  try {
    const response = await axios.post(
      "https://dailysalesapi.skylynxclass.in/api/upload",
      formData,
      {
        headers: {
...
const ClientList = () => {
  ...
  const selectedColumns = ["id", "customerName", "mobileNumber", "refOfFriends", "comment", "date", "isActive" ];
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await clientService.getPaginatedCustomerProfiles({
          page: currentPage,
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        setClients(
          response.data.output.map((client) => ({
            ...client,
            date: client.date
              ? formatDate(new Date(client.date))
              : formatDate(new Date()), // Ensure date is formatted
          }))
        );
      ...
      setClients((prevClients) =>
        prevClients.map((client) =>
          client.id === editedClient.id
            ? {
                ...editedClient,
                date: formatDate(new Date(editedClient.date)),
                photo: photoPath,
              }
            : client
        )
      ...
      setClients((prevClients) => [
        {
          ...
          date: formatDate(new Date(response?.data?.output?.date)),
          photo: photoPath,
        },
        ...prevClients,
      ]);
              ...
              >
                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-xl text-black dark:text-white-dark">
                  ...
                  <div className="p-5">
                    <form
                      onSubmit={
                        editModalOpen ? handleUpdate : handleCreateClient
                      }
                    >
                      {photoPreview && (
