...
const Sidebar = () => {
    ...
    useEffect(() => {
      const fetchMenuData = async () => {
        ...
        const response = await menuService.getRolewiseMenu(user?.roleIds);

        
        if (response.data.isSuccess) {
          setMenuData(response.data.output); // Set the menu data
        }
      };
      fetchMenuData();
    }, []);
                ...
                <div className="bg-white dark:bg-black h-full">
                    <div className="flex justify-between items-center px-4 py-3">
                        <NavLink to="/" className="main-logo flex items-center shrink-0">
                          {themeConfig.menu=='collapsible-vertical' ? <img className="w-8 ml-[5px] flex-none" src={fv_logo} alt="logo" />:<img className="w-40 ml-[5px] flex-none" src={logo} alt="logo" />}
                             </NavLink>
